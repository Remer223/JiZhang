...
Page({
    ...
    wx.request({
      ...
      success: (res) => {
        if (res.data && res.data.code === 200 && res.data.data.hasFamily) {
          this.setData({ myFamilyId: res.data.data.family.id });
        } else {
          this.setData({ myFamilyId: null });
        }
      }
    });
  },
  ...
  submitRecord() {
    ...
    // 如果在家庭中，关联家庭ID
    if (this.data.myFamilyId) {
      submitData.family_id = this.data.myFamilyId;
    }
